...
public class QuestionParserTest {
	...
	public void openData() throws IOException {
		...
		Assert.assertEquals("2011", qr.year);
		Assert.assertEquals("594 (2010-2011) 1", qr.number);
		Assert.assertEquals("29/08/2011 ", qr.question_date);
		Assert.assertEquals("DISABATO Emmanuel", qr.question_from);
		Assert.assertEquals("FURLAN Paul, Ministre des Pouvoirs locaux et de la Ville", qr.question_to);
		Assert.assertEquals("07/10/2011 ", qr.response_date);
		Assert.assertEquals("FURLAN Paul", qr.response_from);
